...
<script>
    ...
    export default {
        ...
        created () {
            //获取“课程类型”数据
            this.$http.post(base.retrival,{"resourceTypeId":"6"}).then((data) => {
                this.ajax.retrivalData = data.data.msgObject;
            });
            //获取“学年”数据
            this.$http.get(base.getAllGrade).then((data) => {
                this.ajax.gradeData = data.data;
            });
        },
        mounted () {
        ...
        watch: {
            ...
            "formStacked.gradeCode" (val, oldVal) {
                this.$http.get(base.getSubjectByGradeCode + val).then((data) => {
                    data.data.unshift({"code": "", "name": "请选择学科"});
                    this.ajax.subjectData = data.data;
                });
            },
            //根据“学年”、“学科”获取“教材”
            "formStacked.subjectCode" (val, oldVal) {
                this.$http.get(base.getBookType + val + "/" + this.formStacked.gradeCode).then((data) => {
                    data.data.unshift({"code": "", "name": "请选择教材"});
                    this.ajax.bookTypeData = data.data;
                });
            }
        },
        components: {
